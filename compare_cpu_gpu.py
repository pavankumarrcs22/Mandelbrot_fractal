...
class ComparisonTool:
    ...
    def side_by_side_comparison(self):
        ...
        # Create resolution-specific subfolder
        resolution_folder = os.path.join(self.images_dir, f"{self.width}x{self.height}")
        if not os.path.exists(resolution_folder):
            os.makedirs(resolution_folder)
        
        print(f"\n[OK] Selected resolution: {self.width}×{self.height}")
        print(f"[OK] Images will be saved to: {resolution_folder}")
        print("\n" + "="*70)
        ...
        for max_iter in iterations:
            ...
            # Adjust figure size based on resolution for proper fit
            ...
            ax_text.text(0.5, 0.5, comparison_text, fontsize=fontsize_text, 
                        verticalalignment='center', horizontalalignment='center',
                        family='monospace',
                        bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5, pad=text_pad))
            
            fig.suptitle(f'CPU vs GPU Comparison - {max_iter} Iterations', 
                        fontsize=fontsize_title, fontweight='bold', y=0.97)
            
            # Save individual figure to resolution-specific subfolder
            filename = os.path.join(resolution_folder, f'comparison_{max_iter}_iterations.png')
    ...
    def dual_interactive_explorer(self):
        ...
        # Set resolution to 800x600 for interactive explorer
        explorer_width, explorer_height = 800, 600
        
        # Renderers
        cpu_renderer = MandelbrotCPU(explorer_width, explorer_height, 256)
        gpu_renderer = MandelbrotRenderer(explorer_width, explorer_height, 256)
...
def main():
    ...
    while True:
        ...
        print("2. Dual interactive explorer")
        print("3. Comparative analysis")
        print("4. Create zoom animation")
